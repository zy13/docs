[同源限制](https://www.wangdoc.com/javascript/bom/same-origin.html)
...
上面的**九个属性**之中，只有`window.location`是可读写的，其他八个全部都是只读。而且，即使是`location`对象，非同源的情况下，也只允许调用`location.replace()`方法和写入`location.href`属性。
